...
linkifyURLs       = 1;
userCSSList       = [ [ 'White On Black', false, false ], [ 'Black on White', true, true ], [ 'Color Terminal', true, true ], [ 'Monochrome', false, false ] ];
	...
	if (typeof userCSSList != 'undefined') {
		...
		for (var i = 0; i <= userCSSList.length; ++i) {
			if (i < userCSSList.length) {
				...
				href.nodeValue                   = '/static/css/usercss-' + i + '.css';
								...
								if (entry.tagName == 'LI') {
									if (++i >= begin) {
										...
										if (i == current) {
											...
											if (!sheet.disabled) {
												label.innerHTML= '<img src="/static/img/enabled.gif" />' +
												label.innerHTML;
											}
										} else {
											sheet.disabled   = true;
			...
			group                             +=
			'<li>' + (enabled ? '<img src="/static/img/enabled.gif" />' : '') +
			label +
			'</li>';
		}
...
VT100.prototype.showContextMenu = function(x, y) {
	...
	'<hr />' +
	'<li id="beginconfig">' +
	(this.utfEnabled ? '<img src="/static/img/enabled.gif" />' : '') +
	'Unicode</li>' +
	'<li id="endconfig">' +
	(this.visualBell ? '<img src="/static/img/enabled.gif" />' : '') +
	'Visual Bell</li>'+
		...
		if (this.isQuestionMark) {
			switch (this.par[i]) {
				case  1:
					...
					break;
				case  3: /* Toggling between 80/132 mode is not implemented */
					break;
				case  5:
					...
					/*;*/ if (ch == 0x3B) {
						...
						switch (ch) {
							/*c*/   case 0x63:
								...
								break;
							/*m*/   case 0x6D: /* (re)set key modifier resource values */
								break;
							/*n*/   case 0x6E: /* disable key modifier resource values */
								break;
							/*p*/   case 0x70: /* set pointer mode resource value */
								break;
							default:
								break;
						}
...
ShellInABox.prototype.sessionClosed = function( status) {
	...
	request.onreadystatechange = function(shellInABox) {
		return function() {
			try {
				...
				shellInABox.sessionClosed( request.status);
			}
		}
	}(this);
